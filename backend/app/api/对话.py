...
def _获取工具调用ID(工具调用: object) -> str:
    if isinstance(工具调用, dict):
        return str(工具调用.get("id") or "")
    return str(getattr(工具调用, "id", "") or "")


def 修复消息历史(消息列表: list) -> list:
    """确保传入图状态的历史消息里不存在未配对的 tool_calls。"""
    原始消息列表 = list(消息列表 or [])
    修复后消息列表 = []

    for 索引, 消息 in enumerate(原始消息列表):
        if not isinstance(消息, AIMessage) or not getattr(消息, "tool_calls", None):
            修复后消息列表.append(消息)
            continue

        存在未配对工具调用 = False
        for 工具调用 in 消息.tool_calls:
            工具调用id = _获取工具调用ID(工具调用)
            if not 工具调用id:
                存在未配对工具调用 = True
                break

            if not any(
                isinstance(后续消息, ToolMessage) and 后续消息.tool_call_id == 工具调用id
                for 后续消息 in 原始消息列表[索引 + 1:]
            ):
                存在未配对工具调用 = True
                break
        ...
        logger.warning("[多Agent] 检测到未配对的 tool_calls，已从历史消息中移除")
        附加参数 = dict(getattr(消息, "additional_kwargs", {}) or {})
        附加参数.pop("tool_calls", None)
        修复后消息列表.append(
            AIMessage(
                content=消息.content,
                name=getattr(消息, "name", None),
                id=getattr(消息, "id", None),
                additional_kwargs=附加参数,
                response_metadata=dict(getattr(消息, "response_metadata", {}) or {}),
                usage_metadata=getattr(消息, "usage_metadata", None),
            )
        )
...
async def 多Agent_SSE生成器(编排配置: dict, agents配置列表: list, 工具记录映射: dict, 消息列表: list, 对话id: str):
    ...
    try:
        ...
        logger.info(f"[多Agent] 开始执行多Agent图，消息列表长度: {len(修复后消息列表)}")
        ...
        try:
            ...
            错误信息 = str(e)
            if "tool_calls" in 错误信息 and "ToolMessage" in 错误信息:
                修复消息历史(修复后消息列表)
                logger.error("[多Agent] 消息历史 tool_call/ToolMessage 不配对: %s", e)
                yield f"data: {json.dumps({'type': 'error', 'content': '多Agent调度异常：子智能体执行失败，请检查所有智能体的LLM配置是否完整（API地址和密钥），然后开始新对话重试'}, ensure_ascii=False)}\n\n"
                return
            raise
